...
- change_intent: adopt the proven odd_sdlc GTL design pattern in odd_domain so that domain build is carried by typed callable assets and named graph functions, with the attribute ledger published as the primary project asset rather than left implicit inside Python orchestration
- change_class: design_reframe
- re_entry_point: design
- triaged_at: 2026-04-16
- priority: high
- dependencies:
- links: basis:20260416T000013Z_STRATEGY_odd-domain-gtl-basis-from-odd-sdlc-review.md
- created_at: 2026-04-16
- updated_at: 2026-04-16

## Context

The current `odd_domain` line had:
...
that GTL pattern in `odd_domain` using `odd_domain` semantics.
...
- how closely `odd_domain` should mirror `odd_sdlc` asset/catalog/query shapes
...
- `odd_domain` now has a local design and implementation line that follows the
...
- the retained GTL realization was created by copying the relevant
  `odd_sdlc` GTL surfaces into `odd_domain` and customizing them locally,
...
- the adopted line defines typed callable asset or node surfaces for:
  ...
  - query projection
- `odd_domain` publishes an explicit machine-readable function catalog and a
...
- `odd_domain.app` bootstraps the workspace event stream through ABG and
...
- the retained deterministic `F_D` evaluators are bound through
  `odd_domain.fd_contracts` and `odd_domain.fd_checks`
