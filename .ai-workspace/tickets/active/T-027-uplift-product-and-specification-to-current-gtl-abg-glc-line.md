# T-027 Uplift Product And Specification To The Current GTL/ABG/GLC Line

- id: T-027
- title: Uplift product and specification to the current GTL/ABG/GLC line
- type: feature
- ticket_category: specification_uplift
- status: active
...
- change_intent: reprice the odd_world_model product definition and requirement families so they are written against the current substrate generation — GTL/ABG 4.6 law as published by abiogenesis 4.6.0-rc.3 and odd_glc 0.1.0 as the generic lifecycle layer WM specializes — replacing the 3.x-era substrate assumptions the specification currently carries
- change_class: product_reprice, requirement_reprice
- re_entry_point: specification/PRODUCT.md
- triaged_at: 2026-07-12
- priority: high
- dependencies: T-026 (active TS rebuild carrier; re-bases on this ticket's grounding)
- links: basis:specification/PRODUCT.md, basis:specification/GOALS.md, basis:specification/INTENT.md, basis:specification/requirements/05-product-definition-authority.md, basis:specification/requirements/50-odd-method-gtl-carrier.md, basis:specification/requirements/95-release-installation-governance.md, basis:build_tenants/TENANT_REGISTRY.md, basis:build_tenants/typescript/design/adrs/ADR-001-typescript-gtl-abg-tech-stack.md, basis:.ai-workspace/tickets/active/T-026-rebuild-world-model-in-typescript-tenant.md, basis:/Users/jim/src/apps/abiogenesis/specification/PRODUCT.md, basis:/Users/jim/src/apps/abiogenesis/docs/ABIOGENESIS_RC_RELEASE_NOTE.md, basis:/Users/jim/src/apps/abiogenesis/specification/requirements/abg/REQ-R-ABG3-EVENTS.md, basis:/Users/jim/src/apps/abiogenesis/.ai-workspace/tickets/completed/T-221-close-exact-4-6-prior-release-boundary.md, basis:/Users/jim/src/apps/odd_glc/specification/PRODUCT.md, basis:/Users/jim/src/apps/odd_glc/.ai-workspace/tickets/completed/T-036-cut-odd-glc-0-1-0.md
- intake_source: F_H owner direction — ground WM's uplift on gtl/abg 4.6 and glc 0.1; WM is not pinned to those versions and picks up latest released implementations; WM builds on top of glc, specialized to a domain; driving use case is `/Users/jim/src/apps/odd_trading_eval/specification` — WM tech builds trade world models from existing code (see Context Capsule, "Driving use case")
- affected_boundary: odd_world_model specification surface versus the published substrate (abiogenesis) and lifecycle-layer (odd_glc) contracts
...
instead of a naive global object graph. Entities are upgraded to statistical
Markov objects — state distributions plus admissible transitions — derived
from source code AND from business process documents and standards, so the
model captures what the system does and what the business lawfully means by
it. The LLM is used as a mapper that projects Markov objects into technology
...
- **ABG**: there is no final 4.6.0. `4.6.0-rc.3` is the exact immutable
  ...
  4.6-generation law rc.3 publishes. The ABG 5.0 line is open (self-hosting
  milestone direction).
- **GLC**: `odd_glc 0.1.0` is released (odd_glc T-036,
  ...
  compatibility range, so WM's floating-pin posture inherits exact-pin
  transitivity through GLC (recorded in Gaps below).
...
- **ABG 4.6 law exists.** The 4.6 generation publishes law WM's spec does not
  ...
  `RUNTIME_EVENT_KIND_VALUES` under contract group `abg.contract.abg.m03`);
  ...
  Operator Contract verb families; and the published-contract schema law
  (exact wire/serialization schemas live in the exact product's public
  contract catalog — runtime shape and source paths are not schema authority).
...
- **Requirements**: families 05/50/95 carry no GLC consumption law, no 4.6
  conformance/census/witness law — stale, but they derive from PRODUCT.
- **Product**: `specification/PRODUCT.md` does not know GLC exists as WM's
  base layer and states no substrate-generation grounding at all. **This is
  the first missing layer.**
- Goals and Intent above it remain valid: proving-wave-07 is explicitly
  specification-first, and INTENT's purpose (externalize the world model
  experts carry in their heads) is untouched by substrate generation.

⇒ **Derived change class**: `product_reprice` + `requirement_reprice`.
⇒ **Re-entry point**: `specification/PRODUCT.md`.

### 4. Affected span

- `specification/PRODUCT.md` (primary re-entry)
...
- **Event census law**: REQ-R-ABG3-EVENTS-029 makes the complete runtime
  ...
  `RUNTIME_EVENT_KIND_VALUES` located by contract group `abg.contract.abg.m03`.
...
- **Operator contract**: abiogenesis publishes one public control plane whose
  verb families are the public named-composition truth (workspace, catalog,
  start/invoke, resume, reads, gaps/lawful-actions, typed F_H operations,
  assess-result, witness, observe, tune, typecheck-gtl-program,
  install/release). `GraphFunction` is the sole named callable catalog
  contribution; workers are invoked only through a selected published graph
  function. (Source: abiogenesis PRODUCT.md, Public Operator Contract and
  Installed And Catalog Product sections.)
- **Published-contract schema law**: exact wire and serialization schemas for
  carriers, envelopes, and payloads are published in the exact product's
  public contract catalog; runtime shape and source paths are not schema
  authority. (Source: abiogenesis PRODUCT.md.)
...
- **Downstream Program Contract**: a downstream program specializes GLC by
  ...
  continuation, replay, and re-entry truth. (Source: GLC PRODUCT.md.)
...
- Spec dated 2026-05-15; installed substrate pin
...
domain specialization of `odd_glc` (0.1 line): GLC owns generic lifecycle
...
- **Markov objects as statistical state/transition carriers**: immutable
  object cuts projected from the attribute ledger, carrying state and
  transition semantics derived from source code, business documents, and
  standards — the self-bounding units through which domain state is reasoned;
- **LLM-as-mapper under F_D-checkable constraint**: probabilistic workers
  (F_P) project Markov objects into technology frames (schemas, API
...
### (b) Grounded on GTL/ABG 4.6-generation law

The uplifted specification SHALL be written against the 4.6-generation
substrate contract captured above: the event census as a conformance
...
published-contract schema law. Requirement family 50 (GTL carrier) is the
...
### (c) Version law — floating pin

The uplifted PRODUCT.md SHALL declare WM's version posture:

> `odd_world_model` is grounded on 4.6-generation GTL/ABG law and the
> odd_glc 0.1 downstream contract. It consumes the **latest released** cuts
> of the substrate and of odd_glc; it is not pinned to an exact version.
> Breakage against a newer released cut is intake pressure for a triaged
> ticket, not a pin regression.

Today "latest released" concretely resolves to `odd_glc 0.1.0` over
`abiogenesis 4.6.0-rc.3` (the exact immutable predecessor — no final 4.6.0
exists, per abiogenesis T-221). The spec SHALL name the law generation it is
written against and cite the published sources; it SHALL NOT freeze exact
versions into product law. Note the asymmetry honestly: GLC 0.1.0 itself
pins exact rc.3 with no compatibility range and no API-stability promise, so
WM's floating-pin posture is a WM-side policy over whatever GLC's latest
release binds, not a promise GLC has made.
...
its remaining design and realization work targets the latest released
substrate/GLC cuts under intent (c), not the 3.7.1-rc.3 install. A re-basing
note is added to T-026 recording this interaction (slice 4). No other part of
T-026 is repriced here.
...
1. `specification/PRODUCT.md` and the affected requirement families
   (05, 50, 95 at minimum; others where the sweep finds pressure) are
   present-tense against the 4.6-generation and GLC-0.1 definitions — no
   "will adopt" language for consumed law that is already published;
...
4. the T-026 re-basing note exists and T-026's metadata reflects it;
5. `build_tenants/TENANT_REGISTRY.md` carries the grounding note;
6. no realization work beyond specification is performed or claimed —
...
- **No final ABG 4.6.0 exists.** rc.3 is the immutable predecessor; the 4.6
  window closed `abandoned_and_rebound`. WM's "latest released substrate"
  resolves to an RC-identity cut until the ABG 5.0 line taps. Owner:
  abiogenesis (T-218/5.0 line). WM spec states the resolution honestly.
- **GLC 0.1.0 publishes no API-stability promise and no compatibility
  range.** WM's floating-pin policy therefore inherits exact-pin transitivity
  through GLC. Owner: odd_glc (future release policy). WM records it as
  declared intake pressure, not compensation.
...
- Slice 1 SHALL extend this table with any consumed contract found
  `missing`, `placeholder`, or unpublished on the exact cuts, following
  GLC's own defer/block/reprice rule rather than compensating locally.
...
2. **PRODUCT reprice** — rewrite `specification/PRODUCT.md`: GLC-downstream
   product position (intent a), 4.6-generation grounding (intent b), version
   law (intent c), preserved WM domain identity (semantic chain, single truth
   surface, federated mesh, Markov objects, LLM-as-mapper under F_D check).
3. **Requirement family reprice** — reprice 05 (grounding and version law as
   product-definition authority), 50 (GTL carrier restated on the 4.6
   surface, per intent e), 95 (installed provenance carries consumed
   substrate/GLC identities); sweep remaining families and reprice only
   where the walk finds pressure.
4. **T-026 re-basing note** — record the interaction of intent (d) in T-026
   and in this ticket's closure evidence.
5. **F_H review** — owner reviews the uplifted specification against this
...
- T-023 mesh and T-024 operational-semantics enrichment scope
