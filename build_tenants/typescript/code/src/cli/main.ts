#!/usr/bin/env node
import { compareExamples, runExamples } from "../sandbox/run_examples.ts";
...
async function main(argv: string[]): Promise<number> {
  ...
  if (command === "run-examples") {
    ...
    const payload = await runExamples(workspace, { runId, clean });
  ...
  console.error("usage: odd-world-model-ts <run-examples|compare-examples> [--workspace .] [--run-id RUN]");
  return 2;
}
