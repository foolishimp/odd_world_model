...
import type { ComparisonResult, ExampleBuilderKind, ExampleDescriptor, ExampleRunResult, JsonObject, SandboxBuilderConfig } from "../../code/src/domain/types.ts";
import { buildDomainInputSeed } from "./domain_input_seed.ts";
import { buildFpmlSourceSeed } from "./fpml_source_seed.ts";
import {
  PROGRAM_STEPS,
  PUBLISHED_AT,
  nowRunStamp,
...
function sandboxConfig(descriptor: ExampleDescriptor): SandboxBuilderConfig {
  const base: Omit<SandboxBuilderConfig, "domain"> = {
...
async function writeRunEvidence(descriptor: ExampleDescriptor, outputs: Record<string, string>): Promise<void> {
  ...
  for (const [index, step] of PROGRAM_STEPS.entries()) {
    ...
    await writeJson(manifestPath, {
      ...
      generated_by: "build_tenants/typescript/historical/filesystem_runner/run_examples.ts"
    });
...
export async function runExamples(workspaceRoot: string, options: { runId?: string; clean?: boolean } = {}): Promise<{ runId: string; results: ExampleRunResult[] }> {
  ...
  for (const descriptor of descriptors) {
    results.push(await runExample(descriptor, options.clean === undefined ? {} : { clean: options.clean }));
  }
