...
import { parseFpmlTrade } from "../../code/src/adapters/fpml_confirmation.ts";
import { runExamples } from "./run_examples.ts";
...
test("example runner emits TypeScript sandbox cuts for every retained example source corpus", async () => {
  ...
  for (const entry of result.results) {
    ...
    const fragmentPath = entry.keyOutputs.fragment;
    const querySummaryPath = entry.keyOutputs.querySummary;
    assert.ok(fragmentPath);
    assert.ok(querySummaryPath);
    const fragment = JSON.parse(await readFile(path.join(entry.sandboxRoot, fragmentPath), "utf8"));
    const querySummary = JSON.parse(await readFile(path.join(entry.sandboxRoot, querySummaryPath), "utf8"));
