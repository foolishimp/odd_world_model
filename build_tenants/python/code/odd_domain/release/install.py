...
"""Install odd_domain into a target project workspace.
...
_BOOTLOADER_END = "<!-- ODD_DOMAIN_BOOTLOADER_END -->"
...
def _workspace_instruction_bootloader(target_root: Path, *, project_slug: str) -> str:
    ...
    return "\n".join(
        (
            "# odd_domain Installed Builder Surface",
            "",
            "This workspace consumes `odd_domain` as an installed domain-builder app.",
            "It is a configured project instance, not the `odd_domain` source workspace.",
            "",
            "## Workspace Identity",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="odd_domain.release.install")
