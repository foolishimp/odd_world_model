...
"""App-owned bootstrap and runtime surface for odd_world_model."""
...
from .gtl_module import module as odd_world_model_module
...
def _app_module(config: AppConfig):
    return config.domain_module or odd_world_model_module(config.workspace_root)
...
def initialize(config: AppConfig, *, worker: Worker | None = None) -> OddWorldModelApp:
    stream = workspace_bootstrap(config.workspace_root)
    return OddWorldModelApp(config=config, stream=stream, worker=worker)


def catalog(app: OddWorldModelApp) -> dict[str, Any]:
...
def gaps(app: OddWorldModelApp) -> dict[str, Any]:
    return gen_gaps(app.scope(), app.stream)


def iterate(app: OddWorldModelApp) -> dict[str, Any]:
    return gen_iterate(app.scope(), app.stream)


def start(app: OddWorldModelApp, *, auto: bool = False) -> dict[str, Any]:
